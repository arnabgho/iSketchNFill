...
class StochasticLabelChannelGatedPix2PixModel(BaseModel):
    def name(self):
        return 'StochasticLabelChannelGatedPix2PixModel'
    ...
    def backward_D(self):
        ...
        pred_fake,_,_ = self.netD(fake_AB.detach(),self.label)
        self.loss_D_fake = self.criterionGAN(pred_fake, False)

        # Real
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        pred_real,_,_ = self.netD(real_AB,self.label)
    ...
    def backward_G(self):
        ...
        pred_fake,q_mu,q_logvar = self.netD(fake_AB,self.label)
        self.loss_G_GAN = self.criterionGAN(pred_fake, True) * self.opt.lambda_GAN

        # Second, G(A) = B
        self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_A

        # Third the information maximization
